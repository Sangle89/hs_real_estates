    ...
    */
    function search() {
        ...
        if($route['search_level'] == 'street') {
            ...
            $this->data['show_tag_link'] = true;
            $this->data['url_pathname'] = $category['alias'] . '-' . $district['alias'];
        }
        ...
        if($route['search_level'] == 'city') {
            ...
            $this->data['title_list_category'] = $category['title'] .' theo quận tại ' . $city['title'];
            $this->data['url_pathname'] = $category['alias'] . '-' . $city['alias'];
        }
        ...
        $this->template->_Set_Data('scroll_to_search', true);
    ...
    */
    ...
    function category_project($category_id, $project_id, $page) {
        ...
        if($project['street_id'] && $project['district_id']) {
            ...
            $this->data['title_search_area'] = $category['title'] . ' theo diện tích tại ' . $district['title'];
            $this->data['title_search_price'] = $category['title'] . ' theo giá tại ' . $district['title'];
        }elseif($project['ward_id'] && $project['district_id']) {
            $district = $this->main_model->_Get_District_By_Id($project['district_id']);
            $this->data['title_search_area'] = $category['title'] . ' theo diện tích tại ' . $district['title'];
            $this->data['title_search_price'] = $category['title'] . ' theo giá tại ' . $district['title'];
        ...
        $this->template->_Set_View('estate/category/list_project', $this->data);
        $this->template->_Render();
    }
    ...
    */
    function real_estate($id) {
       ...
       /* foreach($districts as $district) {
        ...
        $real_estate_content = $this->data['real_estate']['content'];
        $real_estate_content = 	nl2br($real_estate_content);
		$real_estate_content = str_replace("<br /><br />", "<br />", $real_estate_content);
        $real_estate_content = str_replace("<br><br>", "<br />", $real_estate_content);
        $real_estate_content = str_replace("<br>\n<br>", "<br />", $real_estate_content);
        $real_estate_content = preg_replace('/<h3(.*?)>(.*?)<\/h3>/', '', $real_estate_content);
        if($district_id > 0)
            $real_estate_content = $this->replace_content($real_estate_content, $this->data['real_estate'], $category);
        
        $this->data['real_estate']['content'] = $real_estate_content;
    ...
    function replace_content($string, $params, $category) {
        //  Include PHPExcel_IOFactory
        include APPPATH . '/third_party/phpexcel/PHPExcel/IOFactory.php';
            
            $inputFileName = FCPATH . '/excel_data/data.xlsx';
            ...
            //Flag kết đã tìm được 1 từ khóa chưa
            $finded = false;
            $file_district = 'quan_'.$params['district_id'].'.json';
            //echo $file_district;
            $json_data = file_get_contents(FCPATH . '/json_data/street/'.$file_district);
            $array_data = json_decode($json_data);
            $link = base_url($category['alias']);
            foreach($array_data as $key=> $findme) {
                if($key > 0 && $start = strpos($string, $findme)){ //nếu tìm thấy 1 từ khóa trùng
                            $link .= '-duong-'.to_slug($findme).'-'.$params['district_id']; 
                            //$string = str_replace($findme, '<a href="'.$link.'" title="'.$findme.'">'.$findme.'</a>', $string);
                            $string = substr_replace($string, '<a href="'.$link.'" title="'.$findme.'">'.$findme.'</a>', $start, strlen($findme));
                            $finded = true;
                            break;
                }
            }
            ...
            if($finded == false) {
                $json_data = file_get_contents(FCPATH . '/json_data/ward/'.$file_district);
                $array_data = json_decode($json_data);
                
                foreach($array_data as $key => $findme) {
                    if($key>0 && $start = strpos($string, $findme)){ //nếu tìm thấy 1 từ khóa trùng
                                $link .= '-phuong-'.to_slug($findme).'-'.$params['district_id']; 
                                //$string = str_replace($findme, '<a href="'.$link.'" title="'.$findme.'">'.$findme.'</a>', $string);
                                $string = substr_replace($string, '<a href="'.$link.'" title="'.$findme.'">'.$findme.'</a>', $start, strlen($findme));
                                $finded = true;
                                break;
            ...
            /*for ($row = 1; $row <= $highestRow; $row++){ 
                ...
                if($row > 1 && $finded == false) {
                    
                    if($column != '') {
                        $rowData = $sheet->rangeToArray($column . $row . ':' . $highestColumn . $row,
                                                NULL,
                                                TRUE,
                                                FALSE);
                        $rowData = reset($rowData);    
                        $findme = $rowData[0];
                        
                        if($start = strpos($string, $findme)){ //nếu tìm thấy 1 từ khóa trùng
                            $link .= to_slug($findme).'-'.$params['district_id']; 
                            //$string = str_replace($findme, '<a href="'.$link.'" title="'.$findme.'">'.$findme.'</a>', $string);
                            $string = substr_replace($string, '<a href="'.$link.'" title="'.$findme.'">'.$findme.'</a>', $start, strlen($findme));
                            $finded = true;
                        }
            ...
            //Nếu ko tìm thấy kết quả trong sheet đường thì tìm trong sheet phường
            if($finded == false) {
                $sheet = $objPHPExcel->getSheet(1);
                $link = base_url($category['alias'].'-phuong-');
        ...
        $array = array(
         ...
         include APPPATH . '/third_party/phpexcel/PHPExcel/IOFactory.php';
            ...
            foreach($array as $key => $val) {
                ...
                $temp = array();
                for ($row = 1; $row <= $highestRow; $row++){ 
                    $rowData = $sheet->rangeToArray($key . $row . ':' . $highestColumn . $row,
                                                        NULL,
                                                        TRUE,
                                                        FALSE);
                    $rowData = reset($rowData);    
                ...
                file_put_contents(FCPATH . '/json_data/street/'.$val.'.json', json_encode($temp));
            }
