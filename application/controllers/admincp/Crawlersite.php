...
class CrawlerSite extends MY_Admin {
    ...
    function index() {
        ...
        if($post = $this->input->post()) {
            ...
            if ( count($links) > 0 ) {
                ...
                } elseif($post['site'] == 'thuephongtro.com') {
                    foreach($results as $item) {
						$attr = $item->attributes;
						$href = $attr->href;
						$class = isset($attr->class) ? $attr->class : '';
						if(!in_array($href, $array_links))
							$array_links[] = "https://thuephongtro.com".$href;    
					}
                }
    ...
    function import() {
        ...
        } elseif(isset($post['sitename']) && $post['sitename'] == 'thuephongtro.com') {
            $this->import_thuephongtro($post['url'], $post['category_id']);
        }
    }
    ...
    function save_base64_image($file, $name) {
        ...
        if($file!=''){
            ...
            //Resize ảnh gốc 
            $this->image_moo->load($file_fullpath)
                ->set_background_colour('#ffffff')
                ->resize_crop(786, 484, TRUE)
                ->save($file_fullpath, TRUE);
    ...
    function mapCityDistrict($address) {
        ...
        foreach($array_district as $title=>$id) {
            ...
            || strcmp(mb_strtolower($address), mb_strtolower($title)) == 0) {
                $district_id = $id;
                break;   
            }
        }
	...
	function import_kenhnhatro($url, $category_id) {
		...
		if(!empty($data)) {
            ...
            if(!empty($find_price)) {
                $price = $find_price[0]->plaintext;
                $price = str_replace(" vnđ","",$price);
                $price = (int)str_replace(".","",$price);
                $data_price = $this->formatPriceUnit($price);
                $insert['price_number'] = $data_price['price_number'];
                $insert['price_unit'] = $data_price['price_unit'];
                $insert['title'] = $insert['title'].' giá '.$data_price['price_number'] .' '. $data_price['unit_label'];
            }
    ...
    function import_sosanhnha($url, $category_id) {
		...
		if(!empty($data)) {
            ...
            if(!empty($find_price)) {
                $price = str_replace(" vnđ","",$find_price[0]->plaintext);
    ...
    function import_thuephongtro($url, $category_id) {
        require APPPATH . "/third_party/phpcrawl/libs/simple_html_dom.php";
		...
		if(!empty($data)) {
            ...
            //Find title
            $find_title = $data->find('.product-detail h1');
			if(!empty($find_title)) {
				foreach($find_title as $item) {
					$title = $item->plaintext;
                    //Title
            ...
            //info
            $find_info = $data->find('.product-detail .row-desc .row');
            if(!empty($find_info)) {
				//gia tien
				$find_price = $find_info[0]->find('.col2');
                $insert['price_number'] = $find_price[0]->plaintext;
                if(!empty($find_price)) {
                    $price_number = str_replace(" vnđ","",$find_price[0]->plaintext);
                    $price_number = (int)str_replace(",","",$insert['price_number']);
                    $data_price = $this->formatPriceUnit($price_number);
                    $insert['price_number'] = $data_price['price_number'];
                    $insert['price_unit'] = $data_price['price_unit'];
                    $insert['title'] = $insert['title'].' giá '.$data_price['price_number'].' ' . $data_price['unit_label'];
                }
				//Area
				$find_area = $find_info[1]->find('.col2');
				$insert['area'] = $find_area[0]->plaintext;
                if(!empty($find_area)) {
                    $insert['area'] = $find_area[0]->plaintext;
                    $insert['title'] = $insert['title'].' diện tích '.$insert['area'];
				...
				//Address
				$find_address = $find_info[2]->find('.col2');
                if(!empty($address)) {
                    $address = $this->chuanhoaAddress($address);
                    $insert['address'] = $address;
                    $insert['city_id'] = 1;
                ...
                //Fullname
                $find_name = $find_info[3]->find('.col2');
                $insert['guest_fullname'] = $find_name[0]->plaintext;
                //Phone
                $find_phone = $find_info[4]->find('.col2 > span');
                $insert['guest_mobiphone'] = $find_phone[0]->plaintext;
            }
            
            //District
            $find_district = $data->find('.pageBreadcrumbs > .breadcrumb > li');
            if(!empty($find_district)) {
                $find_district = end($find_district);
                $find_district = $find_district->find('a', 0);
                if($find_district) 
                    $find_district = $find_district->find('span', 0);
                if($find_district) 
                    $insert['district_id'] = $this->mapCityDistrict($find_district->plaintext);
            }
       
            $find_content = $data->find('.product-detail > .pd-desc-content');
            if(!empty($find_content)) {
                $insert['content'] = $find_content[0]->innertext;
                $insert['content'] = trim($insert['content']);
                $insert['content'] = str_replace("\n", "<br/>", $insert['content']);
            ...
            if($this->check_exists($title_alias) == false) {
                ...
                //Image
    			$images = array();
                $find_image = $data->find('#myGallery > li');
                $image_id = 0;
                if(!empty($find_image)){
                    foreach($find_image as $item) {
                        $img = $item->find('a', 0);
