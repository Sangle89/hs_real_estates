...
class Link extends MY_Admin {
    public $title = 'Search link';
    ...
    function index($page=0) {
        ...
        $total_rows = $this->link_model->_Count_All();
        $data['category'] = $this->link_model->_Get_All(20, $page);
        ...
        $this->template->_Set_View('link/view_all', $data);
    ...
    function add() {
        ...
        if($this->input->post()) {
            ...
            if($this->form_validation->run()) {
                $this->link_model->_Add($data['post']);
                $this->session->set_flashdata('message', 'Success!');
                redirect(admin_url($this->uri->segment(2)));
            }
        }
        $this->template->_Set_View('link/add', $data);
        $this->template->_Render();
    }
    ...
    function change($id) {
        $data = array();
        $data['title'] = $this->title;
        $data['post'] = $this->link_model->_Get_By_Id($id);
        ...
        if($this->input->post()) {
            ...
            if($this->form_validation->run()) {
                $this->link_model->_Update($data['post'], $id);
                $this->session->set_flashdata('message', 'Success!');
                redirect(admin_url($this->uri->segment(2)));
            }
        }
        
        $this->template->_Set_View('link/change', $data);
        $this->template->_Render();
    }
    
    function delete($id) {
        $this->link_model->_Delete($id);
        redirect(admin_url($this->uri->segment(2)));
    }
}