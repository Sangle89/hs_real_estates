...
class Album extends MY_Admin {
    public $title = 'Album hình ảnh';
    ...
    function index($page=0) {
		...
		if($this->input->post()) {
            foreach($this->input->post('sort_order') as $id => $value) {
                $this->album_model->_Update_Sort($id, $value);
            }
        ...
        $total_rows = $this->album_model->_Count_All();
        $data['category'] = $this->album_model->_Get_All(12, $page);
        ...
        $this->template->_Set_View('album/view_all', $data);
    ...
    function change($id) {
        $data = array();
        $data['title'] = $this->title;
        $data['post'] = $this->album_model->_Get_By_Id($id);
        $data['images'] = $this->album_model->_Get_Array_Image_By_Album($id);
        if($this->input->post()) {
            $data['post'] = $this->input->post();
            ...
            if($this->form_validation->run()) {
                $this->album_model->_Update($data['post'], $id);
                $this->session->set_flashdata('message', 'Success!');
                redirect(admin_url($this->uri->segment(2)));
            }
        }
        $this->template->_Set_Script('UploadImage("images");');
        $this->template->_Set_View('album/change', $data);
        $this->template->_Render();
    }
    
    function delete($id) {
        $this->album_model->_Delete($id);
        redirect(admin_url($this->uri->segment(2)));
    }
}