...
<p id="uploadImage<?=$cache_id?>">
    <img src="<?=base_url('assets/images/bg-upload-image.png')?>" width="100"><br>
    <span>Click để upload ảnh</span>
</p>
<a onclick="removeImage('<?=$cache_id?>');" class="remove_image"><i class="fa fa-times"></i></a>
</div>
<input type="hidden" id="valueImage<?=$cache_id?>" name="images[<?=$cache_id?>]" value="">
...
var btn = document.getElementById('uploadImage<?=$cache_id?>'),
    ...
    var uploader = new ss.SimpleUpload({
        button: btn,
        url: ADMIN_URL + '/common/uploadRealEstate',
        name: 'uploadfile',
...
function removeImage(ElemID) {
    ...
    if($('#valueImage'+ElemID).val() != '') {
        $.ajax({
         url: ADMIN_URL + '/common/deleteAward/' + $('#valueImage'+ElemID).val(),
         type: 'get',
         success: function() {
