...
<div class="sitebar-heading">Nhà cho thuê nổi bật</div>
                    <?php
                            ...
                            foreach($real_estate_featured as $result) { 
                                ...
                                $image_resize = $this->image_model->resize($thumb, 200, 200, 'images');
                            if($result['type_id'] == 1)
                                $class = 'pro';
                            ...
                            <article class="post">
                                ...
                                <div class="properties_grid" style="margin-bottom: 0;border-top: 0;">
                                    ...
                                    <div class="<?=$class?>" style="min-height: inherit;font-size: 14px;font-weight: bold;"><a href="<?=site_url($result['alias'])?>" title="<?=$result['title']?>" rel="nofollow"><?=format_title(sub_string($result['title'],60))?></a></div>
    								</div> <!-- End .properties_title -->
        								<div class="col-md-4 col-xs-4 img_holder">
        									<a href="<?=site_url($result['alias'])?>" title="<?=$result['title']?>" rel="nofollow"> <img src="<?=base_url($image_resize)?>" onerror="this.src='<?=base_url('theme/images/thumb.jpg')?>'" alt="<?=$result['title']?>" class="img-responsive"></a>
                	                   </div> <!-- End .img_holder -->
