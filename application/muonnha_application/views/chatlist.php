...
<html lang="en">
  <head>
    ...
    <!-- Bootstrap core CSS -->
    <link href="<?php echo base_url();?>theme/chat/css/bootstrap.min.css" rel="stylesheet">
    <!-- Bootstrap theme -->
    <link href="<?php echo base_url();?>theme/chat/css/bootstrap-theme.min.css" rel="stylesheet">
	<!-- Font Awesome core CSS -->
    <link href="<?php echo base_url();?>theme/chat/css/font-awesome/css/font-awesome.min.css" rel="stylesheet">
	<!-- Custom styles for this template -->
    <link href="<?php echo base_url();?>theme/chat/css/theme.css" rel="stylesheet">
  ...
  <body role="document">
    ...
    <div class="container theme-showcase" role="main">
		...
		<div class="chatlist">
                ...
                <a href="javascript:;" data-user-id="<?=$row['id']?>" class="btn btn-default" title="Chat with <?=$row['email']?>"><?=$row['email']?></a>&nbsp;
                <?php endforeach; ?>
    ...
    <script src="<?php echo base_url();?>theme/chat/js/bootstrap.min.js"></script>
	<script type="text/javascript" src="<?php echo base_url();?>theme/chat/js/moment.min.js"></script>
	...
	else
	{
		...
		chatpage.on('chatdata',function(data){
			...
			if(data.memdata)
			{
				...
				{
					cells+='<span class="label label-default"><input type="hidden" class="userId" value="'+data.memdata[n].user_id+'"/>'+data.memdata[n].email+'</span>&nbsp;';
				}
			}
			...
			if(data.converdata)
			{
				...
				{
					$('#chat_block_list').append('<li class="media"><div class="media-body"><h4 class="media-heading">'+data.converdata[n].email+'</h4><p>'+data.converdata[n].msg+'</p></div></li>');
				}
			}
			else
			{
		...
		chatpage.on('showcomment',function(data){
			$('#chat_block_list').append('<li class="media"><div class="media-body"><h4 class="media-heading">'+data.room_comment[0].email+'</h4><p>'+data.room_comment[0].msg+'</p></div></li>');
			$('#msg_box').val('');
		});
		chatpage.on('newuser',function(data){
			$('#online-list').append('<span class="label label-default"><input type="hidden" class="userId" value="'+data.userdata[0].user_id+'"/>'+data.userdata[0].email+'</span>&nbsp;') 
		});
