...
$city_id = isset($post['city_id']) ? $post['city_id'] : $city_selected;
$district_id = isset($post['district_id']) ? $post['district_id'] : $district_selected;
$ward_id = isset($post['ward_id']) ? $post['ward_id'] : 0;
$sort_order = isset($post['sort_order']) ? $post['sort_order'] :  $this->street_model->_Get_Max_Sort_Order() + 1;
	...
	<div class="row">
		<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12 sortable-grid ui-sortable">
            <?php 
                ...
                $this->table->add_row(array(
                    'Phường/xã',
                    _Dropdown('ward_id', $ward, $ward_id)
                ));
                $this->table->add_row(array(
