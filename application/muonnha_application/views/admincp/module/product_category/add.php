	...
	<div class="row">
		<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12 sortable-grid ui-sortable">
            <?php 
                ...
                echo form_open_multipart(admin_url('module/'.$this->uri->segment(3) . '/add'), array('method'=>'post','id'=>'myForm', 'class'=>'smart-form')); 
                
                //Vietnamese
                $this->table->add_row(
                    array('data'=>'Title', 'width' => '20%'),
                    array('data' => _Input("content[title]").form_error("content[title]"))
                );
                $this->table->add_row(array(
                    'Danh mục sản phẩm',
                    _Multiselect('content[category][]', $product_category, array())
                ));
                $this->table->add_row(
                     array('Banner 1',
                        _Button_Image('banner1').form_error('banner1')
                ));
                $this->table->add_row(
                     array('Banner 2',
                        _Button_Image('banner2').form_error('banner2')
                ));
                $this->table->add_row(array(
                    array('data'=>'Màu sắc', 'width'=>'20%'),
                    _Input('content[color]')
                ));
                $this->table->add_row(array(
