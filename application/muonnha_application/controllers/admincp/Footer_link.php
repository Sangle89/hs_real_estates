...
class Footer_link extends MY_Admin {
    ...
    function index($page=0) {
		...
		if($this->input->post()) {
            foreach($this->input->post('sort_order') as $id => $value) {
                $this->footer_link_model->_Update_Sort($id, $value);
            }
        ...
        $total_rows = $this->footer_link_model->_Count_All();
        $data['category'] = $this->footer_link_model->_Get_By_Parent();
        ...
        $this->template->_Set_View('footer_link/view_all', $data);
    ...
    function add() {
        ...
        $data['dropdown'] = $this->footer_link_model->_Get_Dropdown();
        if($this->input->post()) {
            ...
            if($this->form_validation->run()) {
                ...
                $this->footer_link_model->_Add($data['post']);
                $this->session->set_flashdata('message', 'Success!');
                redirect(admin_url($this->uri->segment(2)));
            }
        }
        $this->template->_Set_View('footer_link/form', $data);
        $this->template->_Render();
    }
    ...
    function change($id) {
        ...
        $data['title'] = $this->title;
        $data['id'] = $id;
        $data['post'] = $this->footer_link_model->_Get_By_Id($id);
        ...
        if($this->input->post()) {
            ...
            if($this->form_validation->run()) {
                $key = $this->routes_model->_Get_Key_By_Param($data['post']);
                $data['post']['alias'] = $key['key'];
                $this->footer_link_model->_Update($data['post'], $id);
                $this->session->set_flashdata('message', 'Success!');
                redirect(admin_url($this->uri->segment(2)));
            }
        }
    ...
    function delete($id) {
        $this->footer_link_model->_Delete($id);
        redirect(admin_url($this->uri->segment(2)));
    }
}