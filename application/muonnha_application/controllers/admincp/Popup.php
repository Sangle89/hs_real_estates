...
class Popup extends MY_Admin {
    ...
    function index($page=0) {
		...
		if($this->input->post()) {
            foreach($this->input->post('sort_order') as $id => $value) {
                $this->popup_model->_Update_Sort($id, $value);
            }
        ...
        $total_rows = $this->popup_model->_Count_All();
        $data['category'] = $this->popup_model->_Get_All(12, $page);
        ...
        $this->template->_Set_View('popup/view_all', $data);
    ...
    function add() {
        ...
        if($this->input->post()) {
            ...
            if($this->form_validation->run()) {
                $this->popup_model->_Add($data['post']);
                $this->session->set_flashdata('message', 'Success!');
                redirect(admin_url($this->uri->segment(2)));
            }
        }
        $this->template->_Set_View('popup/add', $data);
        $this->template->_Render();
    }
    ...
    function change($id) {
        $data = array();
        $data['title'] = $this->title;
        $data['post'] = $this->popup_model->_Get_By_Id($id);
        ...
        if($this->input->post()) {
            ...
            if($this->form_validation->run()) {
                $this->popup_model->_Update($data['post'], $id);
                $this->session->set_flashdata('message', 'Success!');
                redirect(admin_url($this->uri->segment(2)));
            }
        }
        
        $this->template->_Set_View('popup/change', $data);
        $this->template->_Render();
    }
    
    function delete($id) {
        $this->popup_model->_Delete($id);
        redirect(admin_url($this->uri->segment(2)));
    }
}