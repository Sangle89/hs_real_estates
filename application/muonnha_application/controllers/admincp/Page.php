...
class Page extends MY_Admin {
    ...
    function index($page=0) {
		...
		if($this->input->post()) {
            foreach($this->input->post('sort_order') as $id => $value) {
                $this->page_model->_Update_Sort($id, $value);
            }
        ...
        $total_rows = $this->page_model->_Count_All();
        $data['category'] = $this->page_model->_Get_All(PER_PAGE, $page);
        ...
        $this->template->_Set_View('page/view_all', $data);
    ...
    function add() {
        ...
        if($this->input->post()) {
            ...
            if($this->form_validation->run()) {
                $this->page_model->_Add($data['post']);
                $this->session->set_flashdata('message', 'Success!');
                redirect(admin_url($this->uri->segment(2)));
            }
        }
        $this->template->_Set_View('page/form', $data);
        $this->template->_Render();
    }
    ...
    function change($id) {
        $data = array();
        $data['title'] = $this->title;
        $data['post'] = $this->page_model->_Get_By_Id($id);
        $data['id'] = $id;
        if($this->input->post()) {
            $data['post'] = $this->input->post();
            ...
            if($this->form_validation->run()) {
                $this->page_model->_Update($data['post'], $id);
                $this->session->set_flashdata('message', 'Success!');
                redirect(admin_url($this->uri->segment(2)));
            }
        }
