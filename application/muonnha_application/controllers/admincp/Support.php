...
class Support extends MY_Admin {
    ...
    function index($page=0) {
		...
		if($this->input->post()) {
            foreach($this->input->post('sort_order') as $id => $value) {
                $this->support_model->_Update_Sort($id, $value);
            }
        ...
        $total_rows = $this->support_model->_Count_All();
        $data['category'] = $this->support_model->_Get_All(12, $page);
        ...
        $this->template->_Set_View('support/view_all', $data);
    ...
    function add() {
        ...
        if($this->input->post()) {
            ...
            if($this->form_validation->run()) {
                $this->support_model->_Add($data['post']);
                $this->session->set_flashdata('message', 'Success!');
                redirect(admin_url($this->uri->segment(2)));
            }
        }
        $this->template->_Set_View('support/add', $data);
        $this->template->_Render();
    }
    ...
    function change($id) {
        $data = array();
        $data['title'] = $this->title;
        $data['post'] = $this->support_model->_Get_By_Id($id);
        ...
        if($this->input->post()) {
            ...
            if($this->form_validation->run()) {
                $this->support_model->_Update($data['post'], $id);
                $this->session->set_flashdata('message', 'Success!');
                redirect(admin_url($this->uri->segment(2)));
            }
        }
        
        $this->template->_Set_View('support/change', $data);
        $this->template->_Render();
    }
    
    function delete($id) {
        $this->support_model->_Delete($id);
        redirect(admin_url($this->uri->segment(2)));
    }
}