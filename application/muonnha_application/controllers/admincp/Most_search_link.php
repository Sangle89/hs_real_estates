...
class Most_search_link extends MY_Admin {
    public $title = 'N?i dung chân trang';
    function __construct() {
        parent::__construct();
        $this->load->model(array(
            'most_search_link_model',
            'footer_link_model',
            'category_model',
            'city_model',
            'district_model',
    ...
    function index($page=0) {
		...
		if($this->input->post()) {
            foreach($this->input->post('sort_order') as $id => $value) {
                $this->most_search_link_model->_Update_Sort($id, $value);
            }
        ...
        $total_rows = $this->most_search_link_model->_Count_All();
        
        $data['results'] = $this->most_search_link_model->_Get_All(20, 0);
        ...
        $this->template->_Set_View('most_search_link/view_all', $data);
    ...
    function add() {
        ...
        if($this->input->post()) {
            ...
            if($this->form_validation->run()) {
                $this->most_search_link_model->_Add($data['post']);
                $this->session->set_flashdata('message', 'Success!');
                redirect(admin_url($this->uri->segment(2)));
            }
        }
        $this->template->_Set_View('most_search_link/form', $data);
        $this->template->_Render();
    }
    ...
    function change($id) {
        ...
        $data['title'] = $this->title;
        $data['id'] = $id;
        $data['post'] = $this->most_search_link_model->_Get_By_Id($id);
        $data['dropdown_category'] = $this->category_model->_Dropdown_Category();
        $data['dropdown_city'] = $this->city_model->_Get_Dropdown();
        $data['dropdown_district'] = $this->district_model->_Get_Dropdown();
        $data['dropdown_ward'] = $this->ward_model->_Get_Dropdown();
        ...
        $data['category_selected'] = $this->most_search_link_model->_Get_Selected($id, 'category_id');
        $data['city_selected'] = $this->most_search_link_model->_Get_Selected($id, 'city_id');
        $data['district_selected'] = $this->most_search_link_model->_Get_Selected($id, 'district_id');
        $data['ward_selected'] = $this->most_search_link_model->_Get_Selected($id, 'ward_id');
        $data['street_selected'] = $this->most_search_link_model->_Get_Selected($id, 'street_id');
        ...
        if($this->input->post()) {
            ...
            $this->form_validation->set_rules('links[]', 'Liên kết', 'required|numeric');
            if($this->form_validation->run()) {
                $this->most_search_link_model->_Update($data['post'], $id);
                $this->session->set_flashdata('message', 'Success!');
                redirect(admin_url($this->uri->segment(2)));
            }
        }
    ...
    function delete($id) {
        $this->most_search_link_model->_Delete($id);
        redirect(admin_url($this->uri->segment(2)));
    }
}