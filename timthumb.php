...
if( !isset( $_REQUEST[ "image" ] ) ) { die( "no image specified" ); }

// clean params before use
$src		= preg_replace( "/^(\.+(\/|))+/", "", $_REQUEST['image'] );
...
if($_REQUEST['folder']) {
    $folder = preg_replace( '/^(s?f|ht)tps?:\/\/[^\/]+/i', '', $_REQUEST['folder'] );
} else {
    $folder = '';
}
...
if($folder!='') 
    $src = $_SERVER['DOCUMENT_ROOT'] . '/uploads/'.$folder.'/' . $src;
else
    $src = $_SERVER['DOCUMENT_ROOT'] . '/uploads/images/' . $src;
